        ...
        <template v-if="!currentProgress.locked">
          ...
          <b-row>
            <b-col lg="8" xl="5" offset-xl="4">
              <h5 class="font-weight-bold mb-6">PDG Piston Movement</h5>
            </b-col>
          </b-row>
          <Select
            label="Zero Condition (Visually)"
            placeholder="Select Result"
            v-model="form.pdgPistonMovement.zeroConditionId"
            :options="options.pdgEwsTest"
            :multiple="false"
          />
          <Select
            label="Free Movement of Piston"
            placeholder="Select Result"
            v-model="form.pdgPistonMovement.freeMovementOfPistonId"
            :options="options.pdgEwsTest"
            :multiple="false"
          />
          <b-row>
            <b-col lg="8" xl="5" offset-xl="4">
              <h5 class="font-weight-bold mb-6">dP Switch Activation</h5>
            </b-col>
          </b-row>
          <Select
            label="Warning"
            placeholder="Select Result"
            v-model="form.dpSwitchActivation.warning"
            :options="options.buzzerLight"
            :multiple="false"
          ...
          <InputMoney
            label="Fuel Stop"
            type="number"
            v-model="form.dpSwitchActivation.fuelStop"
            :usePrefix="false"
            append="@15 psi"
          />
          <b-row>
            <b-col lg="8" xl="5" offset-xl="4">
              <h5 class="font-weight-bold mb-6">EWS Loop / Function Test</h5>
            </b-col>
          </b-row>
          <Select
            label="F1"
            placeholder="Select Result"
            v-model="form.ewsLoop.f1Id"
            :options="options.pdgEwsTest"
            :multiple="false"
          />
          <Select
...
import { buzzerLight } from "@/core/datasource/options";
...
export default {
  ...
  data: () => ({
    title: "207 SF - Pressure Differential Gauge & EWS Test",
    route: {
      form: "sf207Create",
      table: "sf207"
    },
    form: {
      ...
      pdgPistonMovement: {
        zeroCondition: null,
        zeroConditionId: null,
        freeMovementOfPiston: null,
        freeMovementOfPistonId: null
      },
      dpSwitchActivation: {
        warning: null,
        fuelStop: null
  ...
  created() {
    ...
    getPdgEwsTest().then(response => {
      self.options.pdgEwsTest = response;
    });
  ...
  methods: {
    ...
    get() {
      ...
      self.$store
        .dispatch("apis/get", {
          url: `/board/standard-form/207/${self.$route.params.id}`
        })
        .then(response => {
          if (response.error) {
            self.$message.error({
            ...
            self.form = {
              ...
              pdgPistonMovement: {
                zeroCondition: response.data.pdgPistonMovement?.zeroCondition,
                zeroConditionId:
                  response.data.pdgPistonMovement?.zeroCondition?.id,
                freeMovementOfPiston:
                  response.data.pdgPistonMovement?.freeMovementOfPiston,
                freeMovementOfPistonId:
                  response.data.pdgPistonMovement?.freeMovementOfPiston?.id
              },
              dpSwitchActivation: {
                warning: response.data.dpSwitchActivation?.warning,
                fuelStop: response.data.dpSwitchActivation?.fuelStop
              },
              ewsLoop: {
                f1: response.data.ewsLoop?.f1,
                f1Id: response.data.ewsLoop?.f1?.id,
                f2: response.data.ewsLoop?.f2,
                f2Id: response.data.ewsLoop?.f2?.id,
                f3: response.data.ewsLoop?.f3,
                f3Id: response.data.ewsLoop?.f3?.id
              },
    ...
    handleSubmit() {
      ...
      if (self.$route.name == self.route.form) {
        ...
        _okText = "Yes, Save";
        _action = "apis/post";
        _url = "/board/standard-form/207";
      } else {
        _confirmText =
          "You are about to update this transaction. Are you sure ?";
        _okText = "Yes, Update";
        _action = "apis/put";
        _url = `/board/standard-form/207/${self.$route.params.id}`;
      }
