...
export default {
  ...
  data: () => ({
    title: "218 SF - Grid Strainer & Strainer Check",
    route: {
      form: "sf218Create",
      table: "sf218",
    },
  ...
  methods: {
    ...
    get() {
      ...
      self.$store
        .dispatch("apis/get", {
          url: `/board/standard-form/218/${self.$route.params.id}`,
        })
        .then((response) => {
          if (response.error) {
            self.$message.error({
    ...
    handleSubmit() {
      ...
      if (self.$route.name == self.route.form) {
        _action = "apis/post";
        _url = "/board/standard-form/218";
      } else {
        _action = "apis/put";
        _url = `/board/standard-form/218/${self.$route.params.id}`;
      }
        ...
        .then((dialog) => {
          self.$store
            .dispatch("apis/download", {
              url: `/board/export/standard-form/218/${self.$route.params.id}`,
            })
            .then((response) => {
              if (response.error) {
                self.$message.error({
                ...
                fileLink.setAttribute("download", "218.xlsx");
