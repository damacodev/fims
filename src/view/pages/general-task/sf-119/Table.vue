<template>
  ...
  >
    <template #toolbar>
      <b-button variant="primary" :to="{ name: 'sf119Create' }" class="mr-2">
    ...
    <template #empty>
      <EmptyTable
        ...
        :href="{ name: 'sf119Create' }"
      />
    </template>
    <template #cell(transactionDate)="data">
      {{ dateFormat(data.value) }}
    </template>
    <template #cell(generalCondition)="data">
      {{ typeof data.value == "string" ? "-" : data.value.label }}
    </template>
  </CardTable>
</template>
...
import { sf119 as columns } from "@/core/datasource/columns";
...
export default {
  ...
  data: () => ({
    title: "119 SF",
    subTitle: "Hydrant Dispenser / Refueller Checklist",
