<template>
  ...
  >
    <template #toolbar>
      <b-button variant="primary" :to="{ name: 'sf139bCreate' }" class="mr-2">
    ...
    <template #cell(harga)="data">
      {{ setRupiah(data.value, 2) }}
    </template>
    <template #cell(klaim.volumeKlaim)="data">
      {{ setVolume(data.value) }}
    </template>
    <template #cell(klaim.nilaiKlaim)="data">
...
import { sf139b as columns } from "@/core/datasource/columns";
...
export default {
  ...
  data: () => ({
    title: "139B SF",
    subTitle: "Formulir Pemeriksaan Bridger Masuk Keluar Lokasi Penerima",
