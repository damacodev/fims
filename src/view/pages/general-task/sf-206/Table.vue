<template>
  <fragment>
    ...
    >
      <template #toolbar>
        <b-button variant="primary" :to="{ name: 'sf206Create' }">
...
export default {
  ...
  data: () => ({
    title: "206 SF",
    subTitle: "Pressure Gauge Acuracy Test Record",
        ...
        .then(dialog => {
          self.$store
            .dispatch("apis/download", {
              url: `/board/export/standard-form/206?dppuId=${
            ...
            .then(response => {
              if (response.error) {
                ...
                fileLink.setAttribute("download", "206.xlsx");
